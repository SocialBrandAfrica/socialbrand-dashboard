...
.NOTES
    ...
    v1.19: Restored the run-level push_log summary row (push_type='sigma_extractor'),
           ...
           (retirement history in canon), that responsibility was orphaned -- nothing
...
function Register-ExtractDeltaTask {
    ...
    # v1.24 (2026-07-17, Pieter ruling: "the daily push needs to happen after EOD").
    # 18:40 read the day BEFORE the 19:00 close and before Sigma's own EOD, so every
    # same-day capture was partial and only self-corrected via $DeltaOverlapDays the
    # NEXT night -- after a human had already read the short figure (BUG-LOG ENG-024).
    # 19:30 sits clear of the close and of Sigma's ~15-minute EOD run.
    # The literal stays hardcoded ONLY because it already was; the correct home is
    # store_extract_config.eod_window_start, which this script already fetches in
    # Get-StoreExtractConfig and ignores here -- an R25 break. Wiring $atTime to that
    # config (and staggering per store from the table) is the real fix, ENG-024, CC's.
    # DO NOT change the task time by hand on a server: this block re-registers the
    # task on every run and will silently put it back to whatever is set here.
    $atTime      = '19:30'
    $atTimeToken = 'T' + $atTime + ':00'
    try {
        ...
        if ($existing) {
            ...
            foreach ($t in $existing.Triggers) {
                if ($t.StartBoundary -and ($t.StartBoundary -match [regex]::Escape($atTimeToken))) { $hasSlot = $true }
            }
            if ($hasSlot) {
