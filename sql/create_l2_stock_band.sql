...
BEGIN
  ...
  pool AS (
    SELECT sp.product_code, sp.daily_ros
    FROM public.l2_stock_position sp
    WHERE sp.store_code = p_store AND sp.class = 'NORMAL'
  ),
  joined AS (
    SELECT p.product_code, COALESCE(p.daily_ros, 0) AS daily_ros,
    ...
    LEFT JOIN public.l2_kvi_profile kvi ON kvi.store_code = p_store AND kvi.product_code = p.product_code
  ...
  computed AS (
    SELECT product_code, daily_ros,
      ...
      COALESCE(kvi_band, 'STANDARD') AS kvi_band,
  ...
  INSERT INTO public.l2_stock_band (
    ...
    shelf_life_cap_skipped, max_band, band_blocked, band_blocked_reason,
  ...
  SELECT 'socialbrand', p_store, product_code, daily_ros, week_index_used, month_factor_used,
    ...
    p_lead_days,
    (rhythm_adjusted_demand * p_lead_days) + (safety_days * rhythm_adjusted_demand) AS min_band,
    p_target_cover_days,
    rhythm_adjusted_demand * p_target_cover_days AS max_band_uncapped,
    gmroi_quartile,
    (COALESCE(gmroi_quartile = 1, false) AND kvi_band NOT IN ('KVI_CRITICAL', 'KVI_IMPORTANT')) AS gmroi_capped,
    true,
    CASE WHEN COALESCE(gmroi_quartile = 1, false) AND kvi_band NOT IN ('KVI_CRITICAL', 'KVI_IMPORTANT')
         THEN (rhythm_adjusted_demand * p_lead_days) + (safety_days * rhythm_adjusted_demand)
         ELSE rhythm_adjusted_demand * p_target_cover_days END AS max_band,
    band_blocked,
