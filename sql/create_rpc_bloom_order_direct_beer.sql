...
CREATE OR REPLACE FUNCTION public.rpc_bloom_order_direct_beer(
...
RETURNS TABLE(
  ...
  ros_window_used text, ros_used numeric, projected_soh numeric, target_days_cover integer,
...
BEGIN
  ...
  base_pool AS MATERIALIZED (
    ...
    WHERE a.store_code = %1$L AND a.merch_group_nr = ANY(cfg.merch_group_nrs)
      AND (COALESCE(s.q364,0) > 0 OR COALESCE(so.soh,0) <> 0)
  ),
  family AS (
  ...
  needc AS (
    SELECT c.*, GREATEST(c.soh_raw,0) - c.ros*%5$s AS proj,
      GREATEST(c.ros*c.target - (GREATEST(c.soh_raw,0) - c.ros*%5$s), 0) AS needu
    FROM calc c
  ),
  packs AS (
    SELECT n.*,
      CASE
        WHEN n.sibling_gated AND n.ros = 0 THEN 1
        WHEN n.ros <= 0 THEN 0
  ...
  SELECT
    ...
    ROUND(pk.ros,4), ROUND(pk.proj,2), pk.target, (pk.needu > 0 OR pk.suggested_packs_calc > 0),
    ...
    format('%%s tier, %%s raw=%%s, SOH %%s, target %%s days -> proj %%s, need %%s = %%s packs%%s%%s',
      pk.tier,
      (CASE pk.tier WHEN 'TOP_100' THEN 'ros_14d' WHEN 'TOP_1000' THEN 'ros_28d' ELSE 'ros_56d' END),
      ROUND(pk.ros,2), pk.soh_raw, pk.target, ROUND(pk.proj,1), ROUND(pk.needu,1), pk.suggested_packs_calc,
  ...
  ORDER BY pk.tier, pk.ros DESC, pk.product_code
