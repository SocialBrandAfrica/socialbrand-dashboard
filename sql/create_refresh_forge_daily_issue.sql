...
-- forge_daily_count_volume_fixed + forge_daily_auto_issue (canon s13 rule 3).
-- =============================================================================
...
-- The overnight diff against posted I/DIWAINV counts (v_forge_count_compliance)
-- then lands issued-vs-counted on the dashboard.
--
-- SCOPE: the fixed volumes are DEMO_CALIBRATION (store #6 sets its own); the
-- issue mechanism and dedupe are GENERAL.
-- =============================================================================
...
-- 3. Morning cron: 04:30 UTC = 06:30 SAST, before trading, after the nightly refresh.
--    (Run in the DB, not committed as re-runnable DDL: re-running cron.schedule with the
--     same name updates the schedule in place.)
