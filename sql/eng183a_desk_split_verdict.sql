...
-- A product linked on more than one desk belongs to the ONE desk whose supplier RECEIPTED it here in
-- desk_receipt_window_days (SEED 182). Both DC and a direct supplier did: DC (Pieter ruling 2026-08-23, until now
-- carried only in rpc_bloom_stock_state's warning; its context, Pieter 2026-09-10: 'DC is the preferred supplier
-- always where there are multiple suppliers'). Neither did: DC first, desk_basis NO_RECEIPT (VERIFY). Read with his
-- 09-10 floor call ('sab is a supplier for certain lines'): multiple suppliers = more than one delivering, by receipts.
...
-- refresh_l2_population_verdict 14cf23ea7807709c7580a09c64d72051 -> b00177e20116a182b09d929577d51093 ; rpc_bloom_stock_state d0921900fb2598240f01ea1295c57a12 -> (warning text only)
...
COMMENT ON COLUMN public.l2_population_verdict.desk_basis IS 'ENG-183 (ORDERING-CANON SSA2). Why the row sits on route_key: SOLE_DESK / RECEIPTS (exactly one linked desk''s supplier receipted it in desk_receipt_window_days) / MULTI_RECEIPT (more than one did: DC, because DC is the preferred supplier always where there are multiple suppliers, Pieter 2026-08-23 and 2026-09-10; else desk_sort) / NO_RECEIPT (none did: DC first on the same ruling, else desk_sort, VERIFY) / SOLE_DESK_OFF_DESK_RECEIPT (one desk, but only a supplier with no desk here receipted it: flagged, nothing moved).';
...
$n3$);
  ...
  src := replace(src, $o4$uc_min numeric, uc_max numeric
    ...
    desk_sort smallint, desk_rcpt boolean
  );$n4$);
  ...
  src := replace(src, $o7$  WITH ranked AS (
    ...
    -- ENG-183 (ORDERING-CANON SSA2): the ONE desk whose supplier RECEIPTED the product owns it.
    -- DC and a direct supplier both receipted: DC (Pieter ruling 2026-08-23). Neither: DC first, VERIFY.
  ...
  src := replace(src, $o11$f.valid_to, v_engine
    FROM f;$o11$, $n11$f.valid_to, v_engine,
         CASE WHEN NOT f.overlap AND NOT f.desk_rcpt AND EXISTS (SELECT 1 FROM _pv_rcpt rr WHERE rr.pc = f.product_code) THEN 'SOLE_DESK_OFF_DESK_RECEIPT'
              ...
              WHEN cardinality(f.rcpt_desks) > 1 THEN 'MULTI_RECEIPT'
              ELSE 'NO_RECEIPT' END,
         f.rcpt_desks
    FROM f;$n11$);
  ...
  src := replace(src, $o14$population-verdict v2.1 (ENG-099 new-range fix)$o14$, $n14$population-verdict v2.2 (ENG-183 desk split by receipts)$n14$);
  IF md5(src) <> 'b00177e20116a182b09d929577d51093' THEN RAISE EXCEPTION 'refresh_l2_population_verdict: patched text md5 is %, expected b00177e20116a182b09d929577d51093', md5(src); END IF;
  EXECUTE src;
  SELECT pg_get_functiondef(p.oid) INTO src FROM pg_proc p JOIN pg_namespace ns ON ns.oid = p.pronamespace WHERE ns.nspname = 'public' AND p.proname = 'refresh_l2_population_verdict';
  IF md5(src) <> 'b00177e20116a182b09d929577d51093' THEN RAISE EXCEPTION 'refresh_l2_population_verdict: installed pin is %, expected b00177e20116a182b09d929577d51093', md5(src); END IF;
END $eng183v$;
...
DECLARE
  ...
  nw text := $ns$linked on more than one desk are each counted on the ONE desk whose supplier receipted them (ORDERING-CANON SSA2, ENG-183), on DC where DC and a direct supplier both did (Pieter ruling 2026-08-23), and on DC flagged VERIFY where neither did.$ns$;
BEGIN
