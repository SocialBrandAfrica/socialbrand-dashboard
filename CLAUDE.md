...
Read("C:\Users\User\Desktop\Daisy\ENGINE-CANON-LAYERS.md")                    # SB-INDEX-021, CORNERSTONE 1 -- the intelligence ladder. §L4 is the grade stamp
Read("C:\Users\User\Desktop\Daisy\ESTATE-MAPS.md")                            # SB-INDEX-026, CORNERSTONE 2 -- a map is generated or it is a memory
Read("C:\Users\User\Desktop\Daisy\RULE-BOOK.md")
Read("C:\Users\User\Desktop\Daisy\SQL-CONVENTIONS.md")                        # SB-INDEX-028, R20 + the join path + prohibited patterns + the constants
...
```
plus the **`ENGINE-CANON-*` children your declared lane touches**, off the CLEANUP-ENGINE-CANON `## 0. SECTION INDEX` — `ENGINE-CANON-CLASSIFICATION` (§1-§11) · `ENGINE-CANON-CAPITAL-AND-CREDITORS` (§12) · `ENGINE-CANON-COUNT-LAW` (§15) · `ENGINE-CANON-CALENDAR` (§16) · `ENGINE-CANON-IDENTITY` (§17) — plus **PIETERSTYLE.md and the `socialbrand-style` skill** every session, plus **`CANON-DOCTRINE.md` (SB-INDEX-029) for any session that WRITES CANON or makes a proposal** and **`SEATS-AND-PIPELINE.md` (SB-INDEX-030) for any session that ROUTES WORK or questions a permission** (the 2026-08-30 §0 subject carve: `§0g`/`§0h`/`§0i`/`§0j` and `§0c`/`§0d`/`§0e` are now REDIRECT STUBS in FILE-GOVERNANCE and the rules live in those two files. **Reading the stub is not reading the rule** — on 2026-09-04 a seat answered a §0g byte-budget question off the stub and reported the whole canonical estate against the START-LOAD budget, the exact counted-set error the 2026-08-31 settlement closed).

**⭐ REFERENCE IS A FIFTH CLASS (PM churned §0d, Pieter ruled 2026-09-04; mirrored here because PM cannot push).** The four-class table read CANONICAL / HANDOVER / BRIEF / WORKING while 17 live files declared REFERENCE, minted by §0g obligation 2 on 2026-07-27. Evidence DEDUCTIVE, whole population. **A REFERENCE file is read ON DEMAND, never at start, and sits OUTSIDE BOTH §0g populations** — neither the 60,000 ceiling nor the 300,000 start-load budget. The `-LINEAGE` / `-LEDGER` companions and the generated `SB-REF-*` family are this class. Nothing was reclassified and no byte figure moved.

**THE COUNTED SET, and both seats got it wrong on 2026-09-04 in opposite directions.** §0g counts **exactly the files §0 step 1 names** — no more, no less. CC included `SQL-CONVENTIONS` (not named in step 1 at the time) and over-reported; PM excluded the `socialbrand-style` skill and under-reported, although §0g names the skill as its own worked example of a registered file counting **wherever it lives**. Measured 2026-09-15 18:12 SAST (CC, `stat -c %s`): **998,730 b across 26 files against 300,000, 3.33×**, the 17 files named in Step 2 plus the five `ENGINE-CANON-*` children, PIETERSTYLE, the skill, CANON-DOCTRINE and SEATS-AND-PIPELINE. The generated reading is `CEILING-CENSUS.md` line 9; quote that or re-measure, never this line. (R28: the 2026-09-04 figure, 890,085 b across 23 files, is `retired_on` 2026-09-15.) State the population with the number, every time. Memory is a pointer list, never a substitute for a file. **SB-VIS-001 is the `.md`** — converted from `.docx` 2026-07-27 night on Pieter's ruling, content unchanged, and the `.docx` plus its `.pdf` render are archived to `archive/evidence/`. Do not reach for either: they are no longer in root and the `.pdf` never rendered reliably anyway.
...
**§0g THE BYTE REPORT — every recital states the measured start-load total against the budget, from the filesystem, never estimated.** The report counts **the on-disk size in bytes of exactly the files the start gate names** — the registered start-load above, plus the `ENGINE-CANON-*` children the lane loads, plus PIETERSTYLE plus the style skill. Nothing else: not LINEAGE/LEDGER companions, not extracted text, not a lane's declined files. **Stated in bytes. KB means 1000, not 1024** — a 1024 divisor understates a 60 KB ceiling by 5%. **The skill counts wherever it lives; a registered file counts because the session carries it, not because of which folder it sits in. SB-VIS-001 counts ONCE, as the registered `.md`.** The current measured figure lives in ONE place, the counted-set paragraph above (2026-09-15, 998,730 b across 26 files), and the generated one in `CEILING-CENSUS.md` line 9. (R28: the 2026-08-27 reading, 879,809 b across 19 files, and the 2026-08-19 reading, 765,340 b, are `retired_on` 2026-09-15. **This line has now been wrong twice. Re-measure, never carry a figure forward.**)

**THE INTERIM — in force ONLY while a start-load file breaches §0g. A declared debt with an owner, not the new normal.** Breaching as at **2026-09-15 18:12 SAST, measured** (`stat -c %s`): **DB-SCHEMA 259,854 b** against 60,000, **the only start-load file over the ceiling.** **CLEARED since the 2026-08-27 list:** RULE-BOOK (59,526 b) and FILE-GOVERNANCE (49,893 b) by the 2026-08-30 subject carves, ORDERING-CANON (59,935 b) by the 2026-08-30 carve and PM's 2026-09-15 §C4 carve. **ORDERING-CANON sits 65 b under the wall and RULE-BOOK 474 b under, so the next write to either owes a carve or a lineage sweep first, never a breach.** DB-SCHEMA has grown, not shrunk, and is the biggest remaining lever (`SB-AUD-DOC-003`, PM's). (R28: the 2026-08-27 list, `DB-SCHEMA 232,852 · ORDERING-CANON 90,275 · RULE-BOOK 88,955 · FILE-GOVERNANCE 84,066`, is `retired_on` 2026-09-15, `superseded_by` this line; earlier readings are in this file's git history. A transcribed measurement has nothing checking it, which is why the generated `CEILING-CENSUS.md` outranks this line.) HANDOVER-CURRENT keeps its own standing rule on top of §0g: it is meant to be read in full every session, so when it grows past a clean one-pass read it is snapshotted to `archive/handovers/` and trimmed back to open items — nothing excluded, only moved. While a breach holds, for a CC build-lane session:
