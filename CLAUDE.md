...
Read("C:\Users\User\Desktop\Daisy\SB-VIS-001_Product_Vision_and_Philosophy.docx")
Read("C:\Users\User\Desktop\Daisy\SB-PRIORITY-FRAMEWORK-001.md")
```
plus **PIETERSTYLE.md and the `socialbrand-style` skill** every session. Memory is a pointer list, never a substitute for a file. **SB-VIS-001 is the `.docx` — that is the file the v2.9 registry names.** The `.pdf` twin is the same document and does not render in every session.
