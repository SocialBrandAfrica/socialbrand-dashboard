...
**THE COUNTED SET, and both seats got it wrong on 2026-09-04 in opposite directions.** §0g counts **exactly the files §0 step 1 names** — no more, no less. CC included `SQL-CONVENTIONS` (not named in step 1 at the time) and over-reported; PM excluded the `socialbrand-style` skill and under-reported, although §0g names the skill as its own worked example of a registered file counting **wherever it lives**. Correct figure at 2026-09-04 11:0x: **890,085 b across 23 files against 300,000, 2.97×**. State the population with the number, every time. Memory is a pointer list, never a substitute for a file. **SB-VIS-001 is the `.md`** — converted from `.docx` 2026-07-27 night on Pieter's ruling, content unchanged, and the `.docx` plus its `.pdf` render are archived to `archive/evidence/`. Do not reach for either: they are no longer in root and the `.pdf` never rendered reliably anyway.
