...
Migrations `eng033_sab_desk_account_scope_config`, `_recipe`, `_stock_state`, `_sales_budget`, `_budget_ledger`. Files `sql/create_rpc_bloom_order_recipe.sql`, `create_rpc_bloom_stock_state.sql`, `create_l2_sales_budget.sql`, `create_refresh_order_budget_ledger.sql`. No schema change (R32 clean: config + function bodies only), no frontend change (`page.jsx` carries labels and the ledger route key, neither of which moved).
